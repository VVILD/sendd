...
class PickupboyResource(Resource):
    ...
    def pending_orders(self, request, **kwargs):
        ...
        alloted_businesses = AddressDetails.objects.filter(Q(pb__phone=pb_ph) & ~Q(status='C') & Q(business__isnull=False)).exclude(order__status='P')
