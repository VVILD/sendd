   ...
   <script type="text/javascript">
    ...
    def save_model(self, request, obj, form, change):
        if obj.pb and obj.status=='Y':
            obj.status='A'
            obj.save()
            address = urllib.quote_plus(str(obj.address))
            phone = urllib.quote_plus(str(obj.pb.phone))
            phone_office = getattr(obj, 'phone_office', ' ')
            phone_mobile = getattr(obj, 'phone_mobile', ' ')
            user_phone = urllib.quote_plus(str(phone_office) + str(phone_mobile))
            order_no = urllib.quote_plus(str(obj.pk))
            name = urllib.quote_plus(str(obj.contact_person))
