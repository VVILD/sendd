    ...
    def clean(self):
        if self.cleaned_data['reverse_pickup_timedate']:
            ...
            if self.cleaned_data['reverse_pickup_timedate'].date() == today:
                if self.cleaned_data['reverse_pickup_timedate'].time() < (
            ...
            if self.cleaned_data['reverse_latest_available_time'] < (
                self.cleaned_data['reverse_pickup_timedate'] + datetime.timedelta(hours=3)).time():
...
class NewTrackingStatus(ModelForm):
    ...
    def clean(self):
        ...
        tracking_list.append(
            {"status": nstatus, "date": str(ttime.strftime("%Y-%m-%d %H:%M:%S")), "location": location})
